...
export const Settings: React.FC<SettingsProps> = ({ onLock }) => {
  ...
  if (loading) {
    return <div style={{ color: "var(--text-med)", padding: "40px", textAlign: "center" }}>Loading settings...</div>;
  }

  return (
    <div className="settings-view">
      <h2 className="view-title" style={{ marginBottom: "24px" }}>⚙️ Application Settings</h2>

      {/* Security Section */}
      <div className="settings-section">
        <h3 className="settings-section-title">Security & Biometrics</h3>
        
        <label className="form-checkbox" style={{ padding: "8px 0" }}>
          <input
            type="checkbox"
          ...
          <div>
            ...
            <div style={{ fontSize: "12px", color: "var(--text-low)", marginTop: "2px" }}>
              Store the database decryption key in your system's secure keyring. Uses native biometrics or system login password to decrypt automatically on startup.
            </div>
          </div>
        </label>
      </div>
      ...
      <div className="settings-section">
        ...
        <div className="form-group">
          <label htmlFor="lock-timeout">Idle Auto-Lock Timeout</label>
          <select
            id="lock-timeout"
          ...
          <p style={{ fontSize: "12px", color: "var(--text-low)", marginTop: "6px" }}>
            The application will automatically lock and clear decrypted keys from memory if no keyboard or mouse activity is detected for the selected period.
          </p>
        </div>
      </div>
      ...
      <div className="settings-section">
        ...
        <div className="form-group">
          <label htmlFor="trash-retention">Auto-Cleanup Period</label>
          <select
            id="trash-retention"
            className="form-select"
          ...
          <p style={{ fontSize: "12px", color: "var(--text-low)", marginTop: "6px" }}>
            Deleted projects and tasks will be kept in the trash and permanently purged from the database after the selected retention period.
          </p>
        </div>
      </div>
      ...
      <div className="settings-section">
        ...
        <button className="btn btn-secondary" onClick={handleManualLock} style={{ width: "fit-content", borderColor: "rgba(239,68,68,0.2)", color: "#ef4444" }}>
          🔒 Lock Database Now
        </button>
      </div>

      {message && (
        <div style={{ textAlign: "center", color: "var(--accent)", fontSize: "14px", fontWeight: "500", marginTop: "16px" }}>
          {message}
        </div>
      )}
    </div>
