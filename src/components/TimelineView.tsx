...
export const TimelineView: React.FC = () => {
  ...
  const fetchTimeline = async () => {
    ...
    try {
      ...
      setError(err?.toString() || "Failed to fetch timeline.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const getStatusGlow = (status: string) => {
    switch (status) {
      case "done": return "var(--accent-glow)";
      case "in_progress": return "var(--primary-glow)";
      default: return "var(--warn-glow)";
    }
  };
  ...
  return (
    <div className="summary-container" style={{ maxWidth: "800px", margin: "0 auto", width: "100%" }}>
      ...
      {loading && updates.length === 0 ? (
        ...
        <div className="glass-panel" style={{ padding: "40px", textAlign: "center", color: "var(--text-med)" }}>
          {searchQuery ? "No matching activity found." : "No task updates logged yet. Progress updates will appear here."}
        </div>
      ) : (
              ...
              <div style={{ display: "flex", flexDirection: "column", paddingLeft: "8px", position: "relative" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                  {groupedUpdates[date].map((update) => (
                    <div key={update.id} style={{ display: "flex", gap: "16px", alignItems: "flex-start", position: "relative" }}>
                      ...
                      <div className="glass-panel" style={{ 
                        ...
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "12px" }}>
                          <h4 style={{ fontSize: "14px", fontWeight: "600", color: "var(--text-high)" }}>
                            {update.task_title}
                          </h4>
                        ...
                        <div style={{ fontSize: "13px", color: "var(--text-med)", wordBreak: "break-word" }}>
                          {update.update_text}
                        </div>
                        ...
                        <div style={{ display: "flex", gap: "8px", marginTop: "4px" }}>
                          ...
                          <span style={{ 
                            fontSize: "10px", 
                            padding: "2px 6px", 
                            background: update.status === "done" ? "var(--accent-glow)" : "rgba(255,255,255,0.04)", 
                            borderRadius: "4px", 
                            color: update.status === "done" ? "var(--accent)" : "var(--text-med)",
                            fontWeight: "600",
                            textTransform: "uppercase"
                          }}>
                            {update.status === "todo" ? "todo" : update.status === "in_progress" ? "in progress" : "done"}
                          </span>
                        </div>
                      </div>
